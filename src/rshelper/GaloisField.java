 ...
 */
public class GaloisField extends Field{
  
  private final int[] logTable;
  private final int[] powTable;
  public final int[][] mulTable;
  public final int[][] divTable;
  public final int[][] addTable;
...
// Field size 256 is good for byte based system
  ...
  public static GaloisField getInstance(int fieldSize,
      int primitivePolynomial, int primitiveElement) {
    int key = ((fieldSize << 16) & 0xFFFF0000) + (primitivePolynomial & 0x0000FFFF);
    GaloisField gf;
    synchronized (instances) {
      gf = instances.get(key);
      if (gf == null) {
        gf = new GaloisField(fieldSize, primitivePolynomial, primitiveElement);
  ...
  public static GaloisField getInstance() {
    return getInstance(DEFAULT_FIELD_SIZE, DEFAULT_PRIMITIVE_POLYNOMIAL, DEFAULT_PRIMITIVE_ELEMENT);
  }

  private GaloisField(int fieldSize, int primitivePolynomial, int primitiveElement) {
    assert fieldSize > 0;
    ...
    addTable = new int[fieldSize][fieldSize];
    int value = 1;
    ...
    // building addition table
    for (int i = 0; i < fieldSize; i++){
    	for (int j = 0; j < fieldSize; j++){
    		
    		assert(i >= 0 && i < getFieldSize() && j >= 0 && j < getFieldSize());
    		addTable[i][j] = i ^ j; // xor macht in dem fall genau die add in einem GF
...
//		int result = power(a, -1);
	  int result = a; 
	  for(int i = 0; i < mulTable[a].length; i++){
		  if(mulTable[a][i] == 1){
			  result = i;
		  }
	  }
  ...
  public int[][] remainder2(int[] dividend, int[] divisor) {
	  ...
	  int[] res = new int[len2];
	    ...
	    for (int i = rem.length - divisor.length; i >= 0; i--) {
	      int ratio =
	        divTable[rem[i + divisor.length - 1]][divisor[divisor.length - 1]];
	      res[i] = ratio;
	      for (int j = 0; j < divisor.length; j++) {
	        int k = j + i;
	        rem[k] = rem[k] ^ mulTable[ratio][divisor[j]];
	      }
	    }
//	   for (int k = 0; k < rem.length; k++){
//		   rem[k] = dividend[k]; 
//	   }
	    return new int[][]{res,rem}; 
	  }
  ...
  /**
   * Substitute x into polynomial p(x). Einsetzen von x in p(x). 
   * @param p input polynomial
   * @param x input field
   * @return p(x)
   */
  ...
  public int getPrimitiveElement() {
