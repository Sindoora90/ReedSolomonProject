...
public class Gauss {
	 ...
	 */
	public static double[] getSolution(double[][] matrix, double[] vector,
			boolean printSteps) {
		// Das Gleichungssystem hat keine eindeutige Loesung!
		if (matrix.length < matrix[0].length) {
		...
		for (int line = 0; line < matrix.length; line++) {
			...
			if (tmpColumn == -1) {
				...
				if (matrix[0].length - 1 >= line) {
					...
					System.out
							.println("Gleichungssystem nicht eindeutig loesbar!");
					return null;
				}
				break;
			}
			...
			for (int row = line + 1; row < matrix.length; row++) {
				...
				// Subtraktion damit unter der Zahl im Umformungsschritt 3 nur
				// nullen stehen
				removeRowLeadingNumber(matrix[row][tmpColumn], line, row,
						matrix, vector);
			}
		}
		...
		for (int column = matrix[0].length - 1; column > 0; column--) {
			...
			for (int row = column; row > 0; row--) {
				...
				// Dazu wird Subtraktion angewandt
				removeRowLeadingNumber(matrix[row - 1][column], column,
						row - 1, matrix, vector);
			}
		}
